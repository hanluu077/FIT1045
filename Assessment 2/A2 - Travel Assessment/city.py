# Task 1 - City [COMPLETED]

from __future__ import annotations #https://peps.python.org/pep-0563/
from typing import Tuple, List
import math
import geopy.distance
from geopy.distance import great_circle as greatest_distance 
...
class City():
    ...
    def __init__(self, name: str, coordinates: Tuple[float, float], city_type: str,\
        ...
        # associate the city with its name in the name_to_cities dictionary
        if self.name in City.name_to_cities:
            City.name_to_cities[self.name].append(self)
        else:
            City.name_to_cities[self.name] = [self]

        # associate the city with its ID in the id_to_cities dictionary
    ...
    def distance(self, other_city: City) -> int:
        ...
        """
        return math.ceil(greatest_distance(self.coordinates, other_city.coordinates).km)
...
def get_city_by_id(city_id: int) -> City | None:
    ...
    """
    return City.id_to_cities.get(city_id, None)
...
def get_cities_by_name(city_name: str) -> List[City]:
    ...
    """
    return City.name_to_cities.get(city_name, [])
