...
class Country():
    ...
    def print_cities(self) -> None:
        ...
        """
        print('Cities of', self)
        sorted_cities = sorted(self.cities, key=lambda c: c.population, reverse=True)
        headers = ["Order", "Name", "Coordinates", "City type", "Population", "City ID"]
        rows = []
        for i, city in enumerate(sorted_cities):
            rows.append([i, city.name, city.coordinates, city.city_type, city.population, city.city_id])
...
def add_city_to_country(city: City, country_name: str, country_iso3: str) -> None:
    ...
    :param city: The city to add to the country
    :param country_name: The name of the country
    :param country_iso3: The unique 3-letter identifier of this country
    :return: None
    """
...
def find_country_of_city(city: City) -> Country:
    ...
    for country in Country.name_to_countries.values():
        if city in country.cities:
            return country
    raise ValueError("City not found in any country")
